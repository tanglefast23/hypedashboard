...
function HoldingsMenu({ data }: { data: HoldingDashboardData }) {
  return <div className="absolute right-0 top-12 z-20 w-80 rounded-2xl border border-slate-700 bg-slate-950/95 p-3 shadow-2xl shadow-black/50 backdrop-blur"><div className="mb-2 flex items-center justify-between"><p className="text-sm font-semibold">Watched holdings</p><Link className="text-xs text-emerald-300" href="/">Home</Link></div><div className="space-y-2">{data.holdings.groups.map((group) => <Link className="block rounded-xl border border-slate-800 bg-slate-900/60 p-3 hover:border-emerald-400/50" href={`/holdings/${encodeURIComponent(group.coin)}`} key={group.coin}><div className="flex items-center justify-between"><span className="font-semibold">{group.position.displayCoin}</span><span className={`mono text-xs ${group.position.side === "LONG" ? "text-emerald-300" : "text-rose-300"}`}>{group.position.side}</span></div><div className="mt-1 flex items-center justify-between text-xs text-slate-400"><span>{formatNumber(group.position.size)} {group.position.displayCoin}</span><span>{formatCompactUsd(group.position.positionValue)}</span></div></Link>)}</div></div>;
}
...
function TwapRow({ twap }: { twap: LiveTwap }) {
  ...
  return <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-3"><div className="mb-2 flex items-start justify-between gap-3"><div><span className={`mono text-xs font-semibold ${sideTone}`}>{twap.side}</span><p className="mt-1 font-semibold">{twap.token}</p></div><div className="text-right"><p className="mono font-semibold">{formatCompactUsd(twap.liveValue)}</p><p className="mono text-sm text-slate-400">{formatNumber(twap.amount)} {displayCoin(twap.token)}</p></div></div><div className="h-2 overflow-hidden rounded-full bg-slate-800"><div className={`h-full ${progressTone} transition-all duration-1000 ease-linear`} style={{ width: `${Math.max(3, twap.liveProgress * 100)}%` }} /></div><div className="mt-2 flex justify-between gap-3 text-xs text-slate-500"><span className="mono truncate">{shortAddress(twap.user)}</span><span className="mono whitespace-nowrap">{formatDuration(twap.liveRemainingMs)} left</span></div></div>;
}
